...
- (instancetype)init
{
    ...
    if (self) {
        ...
        //注册发送消息的通知
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(sentMessageSuccessfull:) name:CUKey.SentMessageSuccessfull object:nil];
        //
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(getMessageReadACK:) name:@"MessageReadACK" object:nil];

        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(n_receiveMessageNotification:) name:DDNotificationReceiveMessage object:nil];
...
-(void)getRecentSession:(void(^)(NSUInteger count))block
{
    ...
    [getRecentSession requestWithObject:@[@(localMaxTime)] Completion:^(NSArray <MTTSessionEntity*>*response, NSError *error) {
        NSMutableArray *array = [NSMutableArray arrayWithCapacity:0];

        for (MTTSessionEntity *session in response) {
            if (session.sessionType == SessionTypeSessionTypeGroup) {
                if ([session.sessionUsers containsObject:TheRuntime.userID] ) {
...
-(void)loadLocalSession:(void(^)(bool isok))block
{
    [[MTTDatabaseUtil instance] loadSessionsCompletion:^(NSArray *session, NSError *error) {
        NSLog(@"loadLocalSession:%@",session);
        [self addSessionsToSessionModel:session];        
        block(YES);
    }];
