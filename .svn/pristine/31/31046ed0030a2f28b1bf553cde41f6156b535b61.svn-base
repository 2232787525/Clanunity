...
//MARK:竖屏全屏模式
- (void)actionVerticalFullScreen {
    self.zoomScreenBtn.selected = !self.zoomScreenBtn.selected;
    if (self.zoomScreenBtn.selected) {
        [self verticalFullScreen];
    }else {
        [self verticalsmallScreen];
    }
}
...
-(void)verticalFullScreen{
    ...
    self.bottomBar.bottom_sd = self.height_sd;
    self.bottomBar.width_sd = self.width_sd;
    self.playerLayer.frame = self.bounds;
    self.playOrPauseBtn.center = self.center;
    [self updateConstraintsIfNeeded];
}

//MARK:竖屏小屏（从竖屏全屏切换到竖屏小屏）
-(void)verticalsmallScreen{
    self.frame = self.playerOriginalFrame;
    self.bottomBar.frame = CGRectMake(0, self.playerOriginalFrame.size.height - bottomBaHeight, self.self.playerOriginalFrame.size.width, bottomBaHeight);
    self.playOrPauseBtn.center = CGPointMake(self.width_sd/2, self.height_sd/2);
...
-(void)setIfverticalScreen:(BOOL)ifverticalScreen{
    _ifverticalScreen = ifverticalScreen;
    [self zoomScreenBtnAction];
}

-(void)zoomScreenBtnAction{
    if (_ifverticalScreen){
        [[NSNotificationCenter defaultCenter]removeObserver:self name:UIDeviceOrientationDidChangeNotification object:nil];
        //竖屏全屏模式
        [_zoomScreenBtn handleEventTouchUpInsideCallback:^{
            [self actionVerticalFullScreen];
        }];
    }else{
        //横屏全屏模式
        [_zoomScreenBtn handleEventTouchUpInsideCallback:^{
...
- (UIView *)bottomBar {
    if (!_bottomBar) {
        ...
        self.label1Left = label1Left;
        self.label1Top = label1Top;
        self.label1Bottom = label1Bottom;
        self.label1Width = label1Width;
