...
/**
 ...
 @param promptContent 提示语：你先对方发送了一个抖动
 */
...
-(void)checkMsgList:(DDChatLoadMoreHistoryCompletion)completion
{
    ...
    [tmp enumerateObjectsUsingBlock:^(MTTMessageEntity *obj, NSUInteger idx, BOOL *stop) {
        if (idx +1 < [tmp count]) {
            ...
            int absMsgID = (int)(obj.msgID - msg.msgID);
            if ( abs(absMsgID) !=1) {
                [self loadHisToryMessageFromServer:MIN(obj.msgID, msg.msgID) loadCount:abs(absMsgID) Completion:^(NSUInteger addcount, NSError *error) {
                    completion(addcount,error);
                }];
            }
        }
