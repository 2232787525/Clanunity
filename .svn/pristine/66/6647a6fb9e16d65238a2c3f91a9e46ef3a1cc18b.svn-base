...
- (UIButton *)doneBtn{
    if (!_doneBtn) {
        ...
        [rightBtn setTitleColor:[UIColor baseColor] forState:UIControlStateNormal];
...
- (UICollectionView *)toolBarThumbCollectionView{
    if (!_toolBarThumbCollectionView) {
        ...
        UICollectionView *toolBarThumbCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(10, 0, self.view.width_sd - 100, 44) collectionViewLayout:flowLayout];
...
- (UILabel *)makeView{
    if (!_makeView) {
        ...
        makeView.frame = CGRectMake(-5, -5, 18, 18);
        makeView.hidden = YES;
...
- (void) setupButtons{
   ...
   UIBarButtonItem *backBar = [[UIBarButtonItem alloc] initWithTitle:@"取消" style:UIBarButtonItemStyleDone target:self action:@selector(back)];
    backBar.tintColor = [UIColor whiteColor];
    self.navigationItem.rightBarButtonItem =backBar;
    
    UIBarButtonItem *leftBar = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"left_white_blackicon"] style:UIBarButtonItemStylePlain target:self action:@selector(backLastVC)];
...
- (void) setupToorBar{
    UIToolbar *toorBar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, 0, KScreenWidth, 44+KBottomStatusH)];
    toorBar.bottom_sd = KScreenHeight;
    ...
    NSString *heightVfl = [NSString stringWithFormat:@"V:[toorBar(60)]-%f-|",KBottomStatusH];
