...
#import "MTTUtil.h"
#define DB_FILE_NAME                    @"clanTT.sqlite"
#define TABLE_MESSAGE                   @"message"
#define TABLE_ALL_CONTACTS              @"allContacts"
#define TABLE_ALL_friends               @"allFriends"
...
#define SQL_CREATE_ALL_FRIENDS      [NSString stringWithFormat:@"CREATE TABLE IF NOT EXISTS %@ (ID text UNIQUE,Name text,Nick text,Avatar text, Department text,DepartID text, Email text,Postion text,Telphone text,Sex integer,updated real,pyname text,signature text)",TABLE_ALL_friends]
...
/**
 ...
 @return sql语句
 */
#define SQL_CREATE_FRIENDS_INDEX        [NSString stringWithFormat:@"CREATE UNIQUE ID on %@(ID)",TABLE_ALL_friends]
...
/**
 ...
 @return sql语句
 */
#define SQL_ADD_FRIENDS_SIGNATURE        [NSString stringWithFormat:@"ALTER TABLE %@ ADD COLUMN signature TEXT",TABLE_ALL_friends]
...
- (void)openCurrentUserDB
{
    ...
    {
        //所有联系人表 更新数据库字段增加signature
        if(![_database columnExists:@"signature" inTableWithName:TABLE_ALL_CONTACTS]){
        ...
        //好友表 更新数据库字段增加signature
        if(![_database columnExists:@"signature" inTableWithName:TABLE_ALL_friends]){
            // 不存在,需要allContacts增加signature字段
            [_database executeUpdate:SQL_ADD_FRIENDS_SIGNATURE];
            // 版本号变0,全部重新获取用户信息
            __block NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
            [defaults setObject:@(0) forKey:@"alllastupdatetime"];
        }
        ...
        //删除好友表
        [self clearTable:TABLE_ALL_friends];
        ...
        [_dataBaseQueue inDatabase:^(FMDatabase *db) {
            ...
            //创建好友表
            if (![_database tableExists:TABLE_ALL_friends]) {
                [self createTable:SQL_CREATE_ALL_FRIENDS];
            }
...
+(NSString *)dbFilePath
{
    NSString* directorPath = [NSString userExclusiveDirection];//path以用户teamid为标识
...
- (NSArray*)messageFromSearchResult:(FMResultSet*)resultSet
{
    ...
    MTTMessageEntity* messageEntity = [[MTTMessageEntity alloc] initWithMsgID:messageID msgType:messageType msgTime:msgTime sessionID:sessionID senderID:fromUserId msgContent:content toUserID:toUserId];
...
-(void)insertMessages:(NSArray *)messages success:(void (^)(void))success failure:(void (^)(NSString *))failure{
...
/**
 ...
 @param completion 结果
 */
-(void)insertAllFriend:(NSArray<MTTUserEntity*>*)friends completion:(InsertsRecentContactsCOmplection)completion{
    [_dataBaseQueue inDatabase:^(FMDatabase *db) {
        [_database beginTransaction];
        __block BOOL isRollBack = NO;
        @try {
            ...
            [friends enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
                ...
                NSString* sql = [NSString stringWithFormat:@"INSERT OR REPLACE INTO %@ VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",TABLE_ALL_friends];
                //ID,Name,Nick,Avatar,Role,updated,reserve1,reserve2
                BOOL result = [_database executeUpdate:sql,user.objID,user.name,user.nick,user.avatar,user.department,user.departId,user.email,user.position,user.telphone,@(user.sex),user.lastUpdateTime,user.pyname,user.signature];
...
/**
 ...
 @param completion 结果 contacts是【MTTUserEntity*】
 */
-(void)getAllFriends:(LoadAllContactsComplection)completion{
    [_dataBaseQueue inDatabase:^(FMDatabase *db) {
        if ([_database tableExists:TABLE_ALL_friends])
        {
            [_database setShouldCacheStatements:YES];
            NSMutableArray* array = [[NSMutableArray alloc] init];
            NSString* sqlString = [NSString stringWithFormat:@"SELECT * FROM %@ ",TABLE_ALL_friends];
            FMResultSet* result = [_database executeQuery:sqlString];
...
/**
 ...
 @param completion 结果
 */
-(void)getFriendFromID:(NSString*)friendID completion:(void(^)(MTTUserEntity *user))completion{
    [_dataBaseQueue inDatabase:^(FMDatabase *db) {
        if ([_database tableExists:TABLE_ALL_friends])
        {
            [_database setShouldCacheStatements:YES];
            
            NSString* sqlString = [NSString stringWithFormat:@"SELECT * FROM %@ where ID= ?",TABLE_ALL_friends];
            FMResultSet* result = [_database executeQuery:sqlString,friendID];
            MTTUserEntity* user = nil;
            while ([result next])
            {
                user = [self userFromResult:result];
