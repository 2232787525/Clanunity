...
//        [self.contentView setBackgroundColor:TTBG];
        ...
        [self.userAvatar.layer setCornerRadius:4];
...
-(void)openProfilePage
{
    ...
    if (![self.currentUserID isEqualToString:[MTTUtil changeOriginalToLocalID:(UInt32)([UserServre shareService].userModel.teamid) SessionType:1]]) {
        //好友
        NSLog(@"%@",self.currentUserID);
        [MBProgressHUD showHUDAddedTo:self.viewController.view animated:YES];
        WeakSelf;
        [ClanAPI requestForUserInfoWithTtUserid:(NSInteger)[MTTUtil changeIDToOriginal:self.currentUserID] result:^(ClanAPIResult * _Nonnull result) {
            ...
            if ([result.status isEqualToString:@"200"]) {
                ...
                if (username != nil) {
                    ...
                    strangerOrFriendVC.username = username;
                    [weakSelf.viewController.navigationController pushViewController:strangerOrFriendVC animated:YES];
            ...
            user.teamid = (NSInteger)[MTTUtil changeIDToOriginal:self.currentUserID];
            user.realname = weakSelf.userName.text;
            memberVC.singleModel = user;
            [weakSelf.viewController.navigationController pushViewController:memberVC animated:YES];
        ...
        //我自己
        MyInfoVC *my = [[MyInfoVC alloc] init];
        [self.viewController.navigationController pushViewController:my animated:YES];
    }
}
...
- (void)setContent:(MTTMessageEntity*)content
{
    ...
    //设置昵称是否隐藏self.session.sessionType == SessionTypeSessionTypeSingle ||
