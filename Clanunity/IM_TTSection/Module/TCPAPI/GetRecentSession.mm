...
- (Analysis)analysisReturnData
{
    Analysis analysis = (id)^(NSData* data)
    {
        ...
        for (ContactSessionInfo *sessionInfo in [rsp contactSessionList]) {
            ...
            char* pOut;
            uint32_t nOutLen;
            uint32_t nInLen = (uint32_t)strlen([string cStringUsingEncoding:NSUTF8StringEncoding]);
            ...
            session.lastFromUserId = sessionInfo.latestMsgFromUserId;
...
- (Package)packageRequestObject
{
    Package package = (id)^(id object,uint16_t seqNo)
    {
        ...
        [req setUserId:(UInt32)[UserServre shareService].userModel.teamid];
