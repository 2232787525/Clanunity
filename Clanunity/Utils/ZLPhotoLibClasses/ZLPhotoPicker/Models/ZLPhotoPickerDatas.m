...
/**PhotoKit  获取所有组对应的图片*/
-(void)getPHAssetCollectionAllGroupWithAllPhotos:(callBackPhotoBlock)callBack{
    
    PHFetchResult *groupAlbumsResult = [PHAssetCollection fetchAssetCollectionsWithType:PHAssetCollectionTypeSmartAlbum subtype:PHAssetCollectionSubtypeAlbumRegular options:nil];
    // 这时 smartAlbums 中保存的应该是各个智能相册对应的 PHAssetCollection
    NSMutableArray *groups = [NSMutableArray array];

    for (NSInteger i = 0; i < groupAlbumsResult.count; i++) {
        // 获取一个相册（PHAssetCollection）
        PHCollection *collection = groupAlbumsResult[i];
        if ([collection isKindOfClass:[PHAssetCollection class]]) {
            PHAssetCollection *assetCollection = (PHAssetCollection *)collection;
            PHFetchResult *result = [PHAsset fetchAssetsInAssetCollection:assetCollection options:nil];
            if (result.count>0) {
                ZLPhotoPickerGroup *pickerGroup = [[ZLPhotoPickerGroup alloc] init];
                pickerGroup.collection = assetCollection;
                pickerGroup.assetsCount = result.count;
                pickerGroup.groupName = assetCollection.localizedTitle;
                pickerGroup.collection = assetCollection;
                [groups addObject:pickerGroup];
            }
