...
- (UIButton *)deleleBtn{
    if (!_deleleBtn) {
        ...
        NSString *widthVfl = @"H:[deleleBtn(deleteBtnWH)]-right-|";
        NSString *heightVfl = @"V:|-margin-[deleleBtn(deleteBtnWH)]";
//        NSDictionary *metrics = @{@"deleteBtnWH":@(50),@"margin":@(10)};
        NSDictionary *metrics = @{@"deleteBtnWH":@(50),@"margin":@(KStatusBarHeight),@"right":@(10)};
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    if (!self.isPush) {
        ...
        if (self.currentPage >= 0) {
            self.collectionView.contentOffset = CGPointMake(self.currentPage * self.collectionView.width_sd , self.collectionView.contentOffset.y);
        }
    }
}
...
- (void)reloadData{
    ...
    if ([self.delegate respondsToSelector:@selector(photoBrowserShowToolBarViewWithphotoBrowser:)]) {
        ...
        CGFloat width = self.view.width_sd;
        CGFloat x = self.view.left_sd;
        if (toolBarView.width_sd) {
            width = toolBarView.width_sd;
        }
        if (toolBarView.left_sd) {
            x = toolBarView.left_sd;
        }
        toolBarView.frame = CGRectMake(x, self.view.height_sd - 44, width, 44);
...
- (void)changeRotationDirection:(NSNotification *)noti{
    ...
    flowLayout.minimumLineSpacing = 0;
    flowLayout.minimumInteritemSpacing = 0;
    flowLayout.itemSize = CGSizeMake(self.view.size_sd.width + ZLPickerColletionViewPadding, self.view.height_sd);
    flowLayout.scrollDirection = UICollectionViewScrollDirectionHorizontal;
